...
import { parseTaipeiInputToUtc, formatUtcToTaipei } from "@/lib/utils/date";
...
async function handleViewSchedule(userId: string, replyToken: string, text?: string, actionType: "direct_open" | "inquiry" = "inquiry") {
  try {
    ...
    if (targetDate) {
      const targetDateObj = dayjs(parseTaipeiInputToUtc(targetDate)).tz("Asia/Taipei").startOf("day").toDate();
    ...
    let message = "";
    if (targetDate) {
      const dateFormatted = formatUtcToTaipei(parseTaipeiInputToUtc(targetDate), "YYYY 年 M 月 D 日");
  ...
  try {
    ...
    if (modificationRequest.action === "modify") {
      ...
      if (modificationRequest.newDueDate) {
        try {
          const newDueDate = parseTaipeiInputToUtc(modificationRequest.newDueDate);
